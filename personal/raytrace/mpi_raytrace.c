...
void Build_mpi_types(MPI_Datatype* input_vec, MPI_Datatype* input_block);
/* */
...
int main(void){
	...
	Build_mpi_types(&MPI_vec, &MPI_block);
...
void Build_mpi_types(MPI_Datatype* input_vec, MPI_Datatype* input_block){
	...
	int array_of_blocklenghts_vec[3] = {1, 1, 1}, array_of_blocklenghts_block[2] = {1, 1};
	MPI_Datatype array_of_types_vec[3] = {MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE};
	// MPI_Aint x_addr, y_addr, z_addr, vec_addr, r_addr;
	MPI_Aint disps_vec[3] = {offsetof(vec,x), offsetof(vec,y), offsetof(vec,z)}, disps_block[2] = {offsetof(block, center), offsetof(block, r)};
	//MPI_Aint array_of_displacements_vec[3] = {0}, array_of_displacements_block[2] = {0};

	/*MPI_Get_address(px, &x_addr);
	MPI_Get_address(py, &y_addr);
	MPI_Get_address(pz, &z_addr);

	array_of_displacements_vec[1] = y_addr - x_addr;
	array_of_displacements_vec[2] = z_addr - y_addr;
	*/
	
	//MPI_Type_create_struct(3, array_of_blocklenghts_vec, array_of_displacements_vec, array_of_types_vec, input_vec);
	...
	MPI_Datatype array_of_types_block[2] = {MPI_vec, MPI_DOUBLE};

	/*
	MPI_Get_address(vec_addr, &pvector);
	MPI_Get_address(r_addr, &pr);

	array_of_displacements_block[1] = r_addr - vec_addr;
	
	MPI_Type_create_struct(2, array_of_blocklenghts_block, array_of_displacements_block, array_of_types_block, input_block);
	*/
